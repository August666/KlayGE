...
namespace KlayGE
{
	...
	void D3D12RenderEngine::ClearTempObjs()
	{
		...
		std::lock_guard<std::mutex> lock(temp_buff_mutex_);
		for (auto iter = curr_render_cmd_allocator_->recycle_after_sync_buffs_.begin();
			iter != curr_render_cmd_allocator_->recycle_after_sync_buffs_.end(); ++ iter)
		{
		...
		curr_render_cmd_allocator_->recycle_after_sync_buffs_.clear();
	}
	...
	void D3D12RenderEngine::RecycleTempBuffer(ID3D12ResourcePtr const & buff)
	{
		if (buff)
		{
			curr_render_cmd_allocator_->recycle_after_sync_buffs_.push_back(buff);
		}
	}
